...
enum SourceType {
  ...
  SourceCategory get category => switch (this) {
    ...
    SourceType.qnap ||
    SourceType.ugreen ||
    SourceType.fnos => SourceCategory.nasDevices,
    // 通用协议
    SourceType.webdav ||
    SourceType.smb ||
    SourceType.ftp ||
    ...
    SourceType.jellyfin ||
    SourceType.emby ||
    SourceType.plex => SourceCategory.mediaServers,
    // 下载工具
    SourceType.qbittorrent ||
    SourceType.transmission ||
    SourceType.aria2 => SourceCategory.downloadTools,
    // 媒体追踪
    SourceType.trakt => SourceCategory.mediaTracking,
    // 媒体管理
    SourceType.nastool ||
    SourceType.moviepilot => SourceCategory.mediaManagement,
    // PT 站点
  ...
  bool get requiresUsername => switch (this) {
    ...
    SourceType.ptSite ||
    // 字幕站点使用 API Key 认证
    SourceType.opensubtitles => false,
    _ => true,
  };
  ...
  /// 获取该分组下的所有源类型
  static List<SourceType> byCategory(SourceCategory category) =>
      SourceType.values.where((type) => type.category == category).toList();
}

/// 源连接状态
enum SourceStatus { disconnected, connecting, requires2FA, connected, error }
    ...
    switch (type) {
      case SourceType.smb:
        ...
        if (shareName != null && shareName.isNotEmpty) {
          ...
          final cleanPath = shareName.startsWith('/')
              ? shareName
              : '/$shareName';
          return cleanPath;
        }
