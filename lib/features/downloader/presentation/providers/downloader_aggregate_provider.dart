...
extension DownloadStatusFilterX on DownloadStatusFilter {
  ...
  bool matches(UnifiedDownloadStatus status) => switch (this) {
    ...
    DownloadStatusFilter.completed => status == UnifiedDownloadStatus.completed,
  };
}
...
class DownloaderThroughputSample {
  const DownloaderThroughputSample({
    required this.downloadSpeed,
    required this.uploadSpeed,
    required this.sampledAt,
  });

  factory DownloaderThroughputSample.from(DownloaderThroughput throughput) =>
      DownloaderThroughputSample(
        downloadSpeed: throughput.downloadSpeed,
        uploadSpeed: throughput.uploadSpeed,
...
final downloaderBootstrapProvider = FutureProvider.autoDispose<void>((
  ref,
) async {
...
final downloaderClientsProvider = Provider.autoDispose<List<DownloaderClient>>((
  ref,
) {
  final sources = ref.watch(downloadToolSourcesProvider);
...
/// 聚合吞吐（运维总览 / 下载器汇总条）。
final downloaderThroughputProvider = Provider.autoDispose<DownloaderThroughput>(
  (ref) {
...
final downloaderThroughputHistoryProvider =
    AutoDisposeNotifierProvider<
      DownloaderThroughputHistoryNotifier,
      List<DownloaderThroughputSample>
    >(DownloaderThroughputHistoryNotifier.new);

class DownloaderThroughputHistoryNotifier
    extends AutoDisposeNotifier<List<DownloaderThroughputSample>> {
  static const _maxSamples = 60;
  Timer? _timer;

  @override
  List<DownloaderThroughputSample> build() {
    ref.onDispose(() => _timer?.cancel());
    _timer = Timer.periodic(const Duration(seconds: 1), (_) => _sample());
    return [
      DownloaderThroughputSample.from(ref.read(downloaderThroughputProvider)),
    ];
  }

  void _sample() {
    final next = DownloaderThroughputSample.from(
      ref.read(downloaderThroughputProvider),
    );
...
bool _isConnected(Ref ref, SourceEntity s) => switch (s.type) {
  SourceType.aria2 =>
    ref.watch(aria2ConnectionProvider(s.id))?.status ==
