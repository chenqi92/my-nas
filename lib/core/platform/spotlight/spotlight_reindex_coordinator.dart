...
class SpotlightReindexCoordinator {
  ...
  /// 重建结果摘要：每类 kind -> 条目数。
  static final lastReportProvider = StateProvider<Map<SpotlightItemKind, int>?>(
    (_) => null,
  );
      ...
      return cache.comics
          .map((c) {
            ...
            return SpotlightItem(
              ...
              title: c.folderName,
              subtitle: c.pageCount > 0
                  ? appL10n.spotlightComicPageCount(c.pageCount)
                  : null,
  ...
  Future<List<SpotlightItem>> _collectNotes() async {
    ...
    try {
      final favorites = await MediaFavoritesService().getAll(
        type: MediaType.note,
      );
      return favorites
          .map((f) {
  ...
  String? _toLocalPath(String? maybeUrl) {
    ...
    if (maybeUrl.startsWith('file://')) {
      return localPathFromFileUri(maybeUrl);
    }
...
final spotlightReindexCoordinatorProvider =
    Provider<SpotlightReindexCoordinator>(SpotlightReindexCoordinator.new);
