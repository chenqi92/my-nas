...
- **已实现**：
  ...
  - ✅ NCM 加密格式解密
  - ✅ 10 段均衡器（Android AudioEffect；桌面端 mpv audio filter）
  - ✅ 可配置交叉淡入淡出
...
- ✅ 笔记目录树形浏览
- ✅ 全文/标题搜索
- ✅ Markdown 编辑、写回、格式工具栏、实时预览与附件上传
- ✅ 保存前检测远端修改，可重新加载或覆盖以解决多设备写入冲突
...
- ✅ Trakt.tv 同步
- ✅ 客户端面部识别（人物聚合）
- 📝 照片地图尚未实现；在实现 GPS EXIF 聚合与地图交互前不暴露产品入口
...
- ✅ 应用锁（PIN/生物识别/后台超时/切换器遮蔽）
- 🚧 隐私政策与商店隐私披露已有仓库草案，待公开 URL、主体信息与最终构建复核
